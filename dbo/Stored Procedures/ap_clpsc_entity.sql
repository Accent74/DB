...
--------------------------------------------
create procedure ap_clpsc_entity
@accid int,
@crcid int,
@start datetime,
@end datetime,
...
insert into #clps
select JOURNAL.J_ENT as ENT, sum(isnull(JRN_CRC.JC_SUM,0)) as SDB, 0 as SCR,
 0 as TDB, 0 as TCR
...
union
select JOURNAL.J_ENT as ENT, 0 as SDB, sum(isnull(JRN_CRC.JC_SUM,0)) as SCR,
  0 as TDB, 0 as TCR
from JOURNAL inner join JRN_CRC on JRN_CRC.J_ID=JOURNAL.J_ID
where JOURNAL.MC_ID=@mcid and J_DONE=2 and J_DATE<@start and JOURNAL.ACC_CR=@accid and JRN_CRC.CRC_ID=@crcid
group by JOURNAL.J_ENT
union 
select JOURNAL.J_ENT as ENT, 0 as SDB, 0 as SCR, sum(isnull(JRN_CRC.JC_SUM,0)) as TDB, 0 as TCR
from JOURNAL inner join JRN_CRC on JRN_CRC.J_ID=JOURNAL.J_ID
...
select JOURNAL.J_ENT as ENT, 0 as SDB, 0 as SCR,
  0 as TDB, sum(isnull(JRN_CRC.JC_SUM,0)) as TCR
from JOURNAL inner join JRN_CRC on JRN_CRC.J_ID=JOURNAL.J_ID
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ap_clpsc_entity] TO [ap_public]
    AS [dbo];
