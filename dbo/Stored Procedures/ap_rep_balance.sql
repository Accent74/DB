----------------------------------------
create procedure ap_rep_balance
@plan int,
@crc int,
@start datetime,
@end datetime,
...
-- start cr
select ACCOUNTS.ACC_ID, Sum(JOURNAL.J_SUM) AS SCR 
into #mto_scr
...
where JOURNAL.MC_ID=@mcid and JOURNAL.J_DONE=2 and ACCOUNTS.ACC_PLAN=@plan and ACCOUNTS.ACC_TYPE>=0 
  and ACCOUNTS.ACC_TYPE < 3 and JOURNAL.J_DATE < @start
group by ACCOUNTS.ACC_ID
-- start db
select ACCOUNTS.ACC_ID, Sum(JOURNAL.J_SUM) as SDB 
into #mto_sdb
...
group by ACCOUNTS.ACC_ID
-- turn cr
select ACCOUNTS.ACC_ID, Sum(JOURNAL.J_SUM) as TCR
into #mto_tcr
from ACCOUNTS inner join JOURNAL on ACCOUNTS.ACC_ID = JOURNAL.ACC_CR
where JOURNAL.MC_ID=@mcid and JOURNAL.J_DONE=2 and ACCOUNTS.ACC_PLAN=@plan and ACCOUNTS.ACC_TYPE>=0 
...
group by ACCOUNTS.ACC_ID
-- turn db
select ACCOUNTS.ACC_ID, Sum(JOURNAL.J_SUM) as TDB
into #mto_tdb
from ACCOUNTS inner join JOURNAL on ACCOUNTS.ACC_ID = JOURNAL.ACC_DB
where JOURNAL.MC_ID=@mcid and JOURNAL.J_DONE=2 and ACCOUNTS.ACC_PLAN=@plan and ACCOUNTS.ACC_TYPE>=0 
and ACCOUNTS.ACC_TYPE < 3 and JOURNAL.J_DATE >= @start and JOURNAL.J_DATE<@end
...
-- common table
select 
  ACCOUNTS.ACC_ID,ACCOUNTS.ACC_TYPE,ACCOUNTS.ACC_S_TYPE,ACCOUNTS.ACC_T_TYPE,ACCOUNTS.ACC_CODE,
...
--нужно включить все счета, поскольку возможна свертка
...
while @@fetch_status = 0
  begin
     ...
     if @acc_saldo = 1 -- collapse by agents
       begin
        exec ap_clps_agent @acc_id, @start, @end, @mcid, @sdb OUT, @scr OUT, @tdb OUT, @tcr OUT, @edb OUT, @ecr OUT
        -- update main table
       ...
       begin
        exec ap_clps_entity @acc_id,@start, @end, @mcid, @sdb OUT, @scr OUT, @tdb OUT, @tcr OUT, @edb OUT, @ecr OUT
        -- update main table
        update #main set SDB=@sdb, SCR=@scr, TDB=@tdb, TCR=@tcr, EDB=@edb, ECR=@ecr
        where ACC_ID=@acc_id
      end
...
select ACC_ID,ACC_TYPE,ACC_S_TYPE,ACC_T_TYPE,SDB,SCR,TDB,TCR,EDB,ECR from #main order by ACC_CODE

GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ap_rep_balance] TO [ap_public]
    AS [dbo];
