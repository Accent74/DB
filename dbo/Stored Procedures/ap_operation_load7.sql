----------------------------------------
create procedure ap_operation_load7
@id1 int,
@id2 int = 0,
@id3 int = 0,
@id4 int = 0,
...
from 
  DOCUMENTS
where 
  DOC_ID=@id1 or DOC_ID=@id2 or DOC_ID=@id3 or DOC_ID=@id4 or
  DOC_ID=@id5 or DOC_ID=@id6 or DOC_ID=@id7
 -- второй ResultSet
select 
  ...
  case when exists(select * from JRN_CRC where JRN_CRC.J_ID=JOURNAL.J_ID) then 1 else 0 end
from 
  JOURNAL
where 
  DOC_ID=@id1 or DOC_ID=@id2 or DOC_ID=@id3 or DOC_ID=@id4 or
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ap_operation_load7] TO [ap_public]
    AS [dbo];
