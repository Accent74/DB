----------------------------------------
create procedure ap_docload2_account
@id int,
@sd datetime,
@ed datetime,
@mcid int
...
where (JOURNAL.ACC_DB=@id or JOURNAL.ACC_CR=@id)and DOCUMENTS.MC_ID=@mcid and DOCUMENTS.DOC_DONE < 100 and
  DOCUMENTS.DOC_DATE>=@sd and DOCUMENTS.DOC_DATE<@ed

GO
GRANT EXECUTE
    ON OBJECT::[dbo].[ap_docload2_account] TO [ap_public]
    AS [dbo];
